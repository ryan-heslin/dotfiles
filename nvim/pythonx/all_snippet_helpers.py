...
# Function signature start and end patterns
reference = {
...
# Search for containing function definition
def get_definition(snip):
    filetype = get_filetype()
    line = int(vim.eval("search('\\s*" + reference[filetype]["start"] + "(', 'bcW')"))
    return line - 1 if line != 0 else None
...
def parse_snippet_count(snip, snippet_string):
    ...
    return 0 if number is None else int(number.group(1))
...
def generate_tabstops(template: str, join="\n", repetitions=1, offset=0):
    ...
    ranges = range(
        offset + 1, offset + tabstops * (repetitions + tabstops - 1), tabstops
    )
    out = []
