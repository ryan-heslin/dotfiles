...
--require("nvim-surround").setup({
    ...
    --surrounds = {
            --["("] = {
                --add = { "( ", " )" },
                --find = function()
                    --return S.get_selection({ textobject = "(" })
                --end,
                --delete = "^(. ?)().-( ?.)()$",
                --change = {
                    --target = "^(. ?)().-( ?.)()$",
            ...
            --["i"] = { -- TODO: Add find/delete/change functions
                --add = function()
                    --local left_delimiter = S.get_input("Enter the left delimiter: ")
                    --local right_delimiter = left_delimiter and S.get_input("Enter the right delimiter: ")
            ...
            --["t"] = {
                --add = function()
                    --local input = S.get_input("Enter the HTML tag: ")
                    --if input then
...
--
                ...
                --end,
                --find = function()
                    --return S.get_selection({ textobject = "t" })
                --end,
                --delete = "^(%b<>)().-(%b<>)()$",
                --change = {
                    --target = "^<([%w-]*)().-([^/]*)()>$",
                    --replacement = function()
                        --local element = S.get_input("Enter the HTML element: ")
                        --if element then
...
--
            ...
            --["b"] = { 
            --add = function()  
               --local container = S.get_input("Enter containing object: ") 
               --if result then 
                --return {{ container .. "["}}
               --end
            --end
            ...
            --["f"] = {
                --add = function()
                    --local result = S.get_input("Enter function name: ")
                    --if result then
                        --return { { result .. "(" }, { ")" } }
                    --end
                --end,
                ...
                --change = {
                    --target = "^.-([%w_]+)()%b()()()$",
                    --replacement = function()
                        --local result = S.get_input("Enter the function name: ")
                        --if result then
                            --return { { result }, { "" } }
                        --end
                    --end,
            ...
            --invalid_key_behavior = {
                ...
                --end,
                --find = function(char)
                    --return S.get_selection({
                        --pattern = vim.pesc(char) .. ".-" .. vim.pesc(char),
                    --})
                --end,
                --delete = function(char)
                    --return S.get_selections({
                        --char = char,
                        --pattern = "^(.)().-(.)()$",
                    --})
                --end,
        ...
        ----invalid_key_behavior = function(char)
            ----vim.api.nvim_err_writeln(
                ----"Error: Invalid surround character " .. S.surround_string(char)
            ----)
        ----end,
        ----pairs = {
            ----["("] = { "( ", " )" },
...
S.get_selections = function(args)
...
S.get_opts = function()
    return vim.b[0].nvim_surround_buffer_opts or S.user_opts
end
...
S.get_add = function(char)
    char = require("nvim-surround.utils").get_alias(char)
    local key = S.get_opts().surrounds[char] or S.get_opts().surrounds.invalid_key_behavior
    return key.add
end
...
S.translate_opts = function(opts)
    ...
    local invalid = opts.surrounds.invalid_key_behavior or S.default_opts.surrounds.invalid_key_behavior
...
S.merge_opts = function(base_opts, new_opts)
    return new_opts and vim.tbl_deep_extend("force", base_opts, S.translate_opts(new_opts)) or base_opts
end

-- Check if a keymap should be added before setting it.
---@param args table The arguments to set the keymap.
S.set_keymap = function(args)
    -- If the keymap is disabled
    if not args.lhs then
        -- If the mapping is disabled globally, do nothing
        if not S.user_opts.keymaps[args.name] then
            return
        end
        -- Otherwise disable the global keymap
        args.lhs = S.user_opts.keymaps[args.name]
        args.rhs = "<NOP>"
    end
...
S.set_keymaps = function(buffer)
    S.set_keymap({
        name = "insert",
        mode = "i",
        lhs = S.get_opts().keymaps.insert,
    ...
    S.set_keymap({
        name = "insert_line",
        mode = "i",
        lhs = S.get_opts().keymaps.insert_line,
        rhs = function()
    ...
    S.set_keymap({
        name = "normal_cur",
        mode = "n",
        lhs = S.get_opts().keymaps.normal_cur,
        rhs = function()
            return "^" .. tostring(vim.v.count1) .. S.get_opts().keymaps.normal .. "g_"
        end,
        opts = {
            buffer = buffer,
    ...
    S.set_keymap({
        name = "normal_line",
        mode = "n",
        lhs = S.get_opts().keymaps.normal_line,
        rhs = function()
    ...
    S.set_keymap({
        ...
        mode = "n",
        lhs = S.get_opts().keymaps.normal_cur_line,
        rhs = function()
            return "^" .. tostring(vim.v.count1) .. S.get_opts().keymaps.normal_line .. "g_"
        end,
        opts = {
            buffer = buffer,
    ...
    S.set_keymap({
        name = "visual_line",
        mode = "x",
        lhs = S.get_opts().keymaps.visual_line,
...
S.setup = function(user_opts)
    -- Overwrite default options with user-defined options, if they exist
    S.user_opts = S.merge_opts(S.translate_opts(S.default_opts), user_opts)
    -- Configure global keymaps
    S.set_keymaps(false)
    -- Configure highlight group, if necessary
    if S.user_opts.highlight.duration then
        vim.cmd([[
...
S.buffer_setup = function(buffer_opts)
    -- Merge the given table into the existing buffer-local options, or global options otherwise
    vim.b[0].nvim_surround_buffer_opts = S.merge_opts(S.get_opts(), buffer_opts)
